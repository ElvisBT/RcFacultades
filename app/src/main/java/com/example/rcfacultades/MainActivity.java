package com.example.rcfacultades;
...
import com.example.rcfacultades.ml.Modelofacultades;
...
public class MainActivity extends AppCompatActivity implements ImageReader.OnImageAvailableListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btCamara = findViewById(R.id.btObject);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        for(int i=0; i<permissions.length; i++){
            if(permissions[i].equals(android.Manifest.permission.CAMERA)){
                btCamara.setEnabled(grantResults[i] == PackageManager.PERMISSION_GRANTED);
        ...
        if (Build.VERSION.SDK_INT >= 23)
            for(String permiso: listaPermisos) {
                ...
                if(permiso.equals(android.Manifest.permission.CAMERA))
                    btCamara.setEnabled(habilitado);
